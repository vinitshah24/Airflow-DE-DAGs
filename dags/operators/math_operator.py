...
class MyBasicMathOperator(BaseOperator):
    ...
    def __init__(self, first_number: float, second_number: float, operation: str, *args, **kwargs,):
    ...
    def execute(self, context):
        self.log.info(f"Equation: {self.first_number} {self.operation} {self.second_number}")
        if self.operation == "+":
        ...
        if self.operation == "/":
            try:
                ...
                self.log.critical("If you have set up an equation where you are trying to divide "
                                  "by zero, you have done something WRONG. - Randall Munroe, 2006")
