...
def _generate_events(end_date):
    """Generates a fake dataset with events for 150 days before end date."""
    events = pd.concat(
        [_generate_events_for_day(date=end_date - timedelta(days=(150 - i))) for i in range(150)],
        axis=0,
    )
    return events
...
app = Flask(__name__)
app.config["events"] = _generate_events(end_date=date(year=2023, month=12, day=30))
